...
Текущая версия исходников: **0.1.0** · MVP локальных окружений · Web + PWA · GitHub Pages опубликован
...
7. Замкнутая пустая область — **домик**, а не результативное окружение.
8. По классическим правилам области окружения можно окружать снова, освобождая ранее захваченные свои точки.
...
- счёт рассчитывается по активным захваченным точкам;
- внутри захваченной области нельзя ставить новые точки;
...
- Vitest, CI и автоматическая публикация в GitHub Pages;
- закрытая proprietary-лицензия All Rights Reserved.

Базовое окружение уже реализовано. Продвинутые случаи классических правил — вход в домик, вложенные окружения, окружение чужого окружения и освобождение ранее захваченных точек — ещё находятся в разработке.
...
│   ├── board.test.ts      интеграционные тесты игрового ядра
│   ├── capture.ts         определение окружений и подсчёт захвата
...
Текущая архитектура механики окружений и оставшиеся сложные случаи описаны в [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md).
...
1. Завершить продвинутую классическую механику: вход в домик, вложенные окружения, освобождение точек и сложные конкурирующие контуры.
2. Расширить тесты топологии и усилить проверку выбора минимальной области.
